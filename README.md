...
`--reverse` (route mode only): after the last waypoint, fly the same route backwards to the first before looping — "there and back" instead of teleporting straight back to the start. The reversed leg between two waypoints uses that leg's `outboundAltitude` — the altitude *departing* a point, flying forward, is the altitude *arriving* at that same point, flying in reverse — and a freshly-computed reciprocal bearing (not just the forward heading + 180, which only exactly matches on a rhumb line).
